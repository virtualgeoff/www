...
*
*  Have added constraints, max and min values, snapping, callback function
...
dragDrop = {
	...
	mousedown : function(e) {
		...
		if (window.getComputedStyle) {
			// gets initial styles if set by stylesheets (DOM)
			...
			element.initX = parseInt(getComputedStyle(element, null).left);
			element.initY = parseInt(getComputedStyle(element, null).top);		
		} else {
			// IE < 9;
 			element.initX = parseInt(element.currentStyle.left);
			element.initY = parseInt(element.currentStyle.top);		
		}
