...
Next:
	...
	- fix bugs:
		- segment drawing is borked at high latitudes when changing location, but fixes on reload
		- need to store geoLocation so browser doesn't ask again each time you uncheck 'set location manually'

	- styling:
		√ absolute positioning for overlays
		√ make option overlay narrower?
		- background gradient for clock face
		- better icons for about, options

	- about:
		- about sun clock
		- why does it go backwards? why would I want this?
		- what are the different twilights?

	- server:
		- need server with https to use Geolocation API!
		- make dedicated repo and use Github pages?
		- need server to test on mobile

	- mobile:
		- touch events
		- position info better
		- mobile styles

	- future:
		- add moon position and phase!
			- hover over moon to get rise and set time, and phase
		- make it 3D: tilt clock face depending on latitude, rotate whole clock depending on direction (mobile only?)
...
var SunClock = (function() {
	...
	function drawTimeSegments() {
		...
		// fill clock face depending on whether sun rises
		//if (noonPosition.altitude > 0) {
		//	$('#clockFace').setAttribute('fill', '#dceaff');
		//}
	...
	function drawNumbers(parent, n, offset, startAtTop) {
		...
		for (let i=0; i<=(n-1); i++) {
			...
			/* if ((n===24) && (i===6)) {
				m.innerHTML = `<text x="${(h*0.375*-direction)}" y="${(h*0.375)}" transform="rotate(${-90*direction})">E</text>`;  // East
			} else if ((n===24) && (i===18)){
				m.innerHTML = `<text x="${(h*0.375*direction)}" y="${(h*0.375)}" transform="rotate(${90*direction})">W</text>`;  // West
			} else */
	...
	function loadOptions() {
		...
		if (getItem('antiClockwise') === null) {
			// direction is unset — set later based on latitude
		} else {
